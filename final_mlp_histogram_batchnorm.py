...
np.save('experimental_dataset_em.npy', experimental_dataset)
experimental_dataset = np.load('experimental_dataset_em.npy', allow_pickle=True)
...
# experimental_dataset_df = over_sampler(experimental_dataset_df, 'Romance', isolate_col=True, dont_duplicate_these_genres=['Drama'], duplication_ratio=2.0)
...
class MLP_Histogram_Batchnorm(nn.Module): # Compared to MLP-Histogram: More hidden layers. Greater number of nodes to begin with (512 vs 128), and Batch normalisation after each linear layer
    def __init__(self):
        super(MLP_Histogram_Batchnorm, self).__init__()
        self.layers = nn.Sequential(
            nn.Linear(7500, 512),  
            nn.BatchNorm1d(512),  # Added Batch Normalisation
            nn.ReLU(),
            nn.Dropout(0.5),

            nn.Linear(512, 256),  # Increased neurons
            nn.BatchNorm1d(256),  # Added Batch Normalisation
            nn.ReLU(),
            nn.Dropout(0.5),

            nn.Linear(256, 128),
            nn.BatchNorm1d(128),  # Added Batch Normalisation
            nn.ReLU(),
            nn.Dropout(0.5),
...
def pytorch_mlp(dataset, targets, genres):
    ...
    model = MLP_Histogram_Batchnorm().to(device)
    ...
    torch.save(model.state_dict(), 'mlp_histogram_bn_trained_model.pth')
...
model = MLP_Histogram_Batchnorm().to(device)
model.load_state_dict(torch.load('mlp_histogram_bn_trained_model.pth'))
model.eval()
...
mlp_histogram_test_report_all_folds.to_csv('mlp_histogram_bn_test_report_all_folds.csv')
mlp_histogram_test_report_folds_average_df.to_csv('mlp_histogram_bn_test_report_folds_average.csv')
...
full_genre_counts.to_csv("full_genre_counts_mlp_histogram_bn.csv")
